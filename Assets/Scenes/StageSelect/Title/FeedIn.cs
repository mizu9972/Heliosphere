...
public class FeedIn : MonoBehaviour
{
    ...
    public void Init(float _setStartAlpha, float _setEndAlpha, float _setFeedInTime)
    {

        _StartAlpha = _setStartAlpha / 255;
        _EndAlpha = _setEndAlpha / 255;
        SaveAlpha = _StartAlpha;//SaveAlpha初期化
        subAlpha = _EndAlpha - _StartAlpha;
        FeedInSpeed = subAlpha / _setFeedInTime;

        //StartAlphaとEndAlphaの値の上下関係でフェードインかフェードアウトか判定
        if (_setStartAlpha < _setEndAlpha)
        {
            isFeedInorOut = Feed.In;
        }
        else if (_setStartAlpha > _setEndAlpha)
        {
        ...
        var FeedInJudgeActive = this.UpdateAsObservable()//・・・・ストリーム１
            ...
            .Where(_ => SaveAlpha >= _setEndAlpha);//StartAlphaが増加してEndAlpha以上

        var FeedOutJudgeActive = this.UpdateAsObservable()//・・・・ストリーム２
            .Where(_ => isFeedInorOut == Feed.Out)//フェードアウトの場合
            .Where(_ => SaveAlpha <= _setEndAlpha);//StartAlphaが減少してEndAlpha以下
